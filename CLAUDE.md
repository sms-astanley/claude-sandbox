...
- **GSD's root helper tree was renamed `get-shit-done/` → `gsd-core/` in 1.8.0.** The entrypoint links whichever name exists *and* removes a leftover symlink under the other name, because a volume upgraded across that rebuild would otherwise keep a dangling `~/.claude/get-shit-done`. Root-level links aren't covered by the `find -name 'gsd-*' -type l -delete` sweep (that only runs inside `skills/commands/agents/hooks`), which is why the rename needs explicit handling. Don't link `/opt/gsd/scripts/` — those are GSD's own build scripts (changesets, codegen), not runtime, and `scripts` is too generic a name to plant in `~/.claude`.
